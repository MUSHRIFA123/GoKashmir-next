...
export default function HomepageVideos({ videos }) {
  const [currentIndex, setCurrentIndex] = useState(0);
  const videosPerView = 4; // number of videos visible
  const videoWidthPercent = 22; // adjust for gap

  const totalVideos = videos.length;

  // Carousel navigation
  const handleNext = () => {
    if (currentIndex < totalVideos - videosPerView) setCurrentIndex(currentIndex + 1);
  };
    ...
    <div className="relative max-w-7xl mx-auto px-4 py-10">
      ...
      <div className="overflow-hidden relative">
        ...
        <div
          className="flex transition-transform duration-300 gap-4"
          style={{
            transform: `translateX(-${currentIndex * (videoWidthPercent + 2)}%)`,
          }}
        >
          {videos.map(video => (
            <div
              ...
              style={{ width: `${videoWidthPercent}%` }} // safe dynamic width
            >
              <VideoCard video={video} />
            </div>
          ))}
