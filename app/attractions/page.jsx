...
// import AttractionsGuide from "@/data/json/AttractionsGuide";
...
//   // Locations from data
//   const locations = [...new Set(AttractionsGuide.map((a) => a.location))];
...
//   // Filtering logic
//   const filteredAttractions = AttractionsGuide.filter((item) => {
...
export default function AttractionsPage() {
  ...
  useEffect(() => {
    fetch("/api/places")
      .then((res) => res.json())
      .then((data) => {
        const attractions = data.filter((p) => p.category_slug === "attractions");
  ...
  const filtered = places.filter((p) => {
    ...
    if (selectedLocations.length && !selectedLocations.includes(p.locality)) return false;
    return true;
  });
  ...
  return (
    ...
    <section className="py-20 bg-white">
      <div className="container mx-auto px-4">
        <h1 className="text-4xl font-bold mb-8 text-center">Explore Attractions</h1>

        <div className="flex flex-col lg:grid lg:grid-cols-4 gap-8">
          {/* Sidebar */}
          <aside className="w-full lg:col-span-1 lg:border-r lg:pr-6">
            <div className="bg-white rounded-xl p-4 shadow lg:shadow-none lg:sticky lg:top-24">
              <h3 className="font-semibold text-lg mb-3">Top Attractions</h3>
              <label className="flex items-center gap-3 text-gray-700">
                <input
                  type="checkbox"
                  checked={showTopOnly}
              ...
              <h3 className="font-semibold text-lg mt-6 mb-3">Locations</h3>
              <div className="grid grid-cols-2 gap-3 lg:grid-cols-1">
                {locations.map((loc) => (
                  <label key={loc} className="flex items-center gap-2 text-gray-700">
                    <input
                      type="checkbox"
                      checked={selectedLocations.includes(loc)}
                      onChange={() => handleLocationChange(loc)}
              ...
              {filtered.map((item) => (
                ...
                >
                  ...
                  >
                    <FaHeart
                      className={`text-lg ${
                        isFavorite(item.slug) ? "text-red-500" : "text-gray-300"
                      }`}
                    />
                  </button>

                  {/* Card */}
                  <Link href={`/attractions/${item.slug}`} className="block w-full h-full">
                    <img
                      src={item.image}
                      alt={item.title}
                      className="w-full h-full object-cover group-hover:scale-110 transition duration-500"
                    ...
                    <div className="absolute inset-0 bg-gradient-to-t from-black/70 to-transparent opacity-0 group-hover:opacity-100 transition flex items-end p-4">
                      <div>
                        ...
                        <p className="text-sm text-gray-200">{item.locality}</p>
                      </div>
                    </div>
                  </Link>
                </div>
